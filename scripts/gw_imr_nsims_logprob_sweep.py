...
IMRPhenomD variant: swaps the TaylorF2 inspiral-only waveform in
``scripts/gw_waveform_nsims_logprob_sweep.py`` for ``IMRPhenomD`` via pycbc
(full inspiral-merger-ringdown, non-spinning). All other conventions match the
TaylorF2 sweep script: masses are MinMaxScaled to [0.1, 1.0], the same analytic
aLIGO PSD and unit-variance whitened-domain noise are used, and the same eta
and conventional-coordinate baselines are trained for comparison.

Intended to be run as a command-line job, for example in Google Colab:

    python scripts/gw_imr_nsims_logprob_sweep.py --out-dir gw_imr_sweep_results
...
try:
    ...
    raise SystemExit(
        "This script requires pycbc (+ lalsuite) for IMRPhenomD waveforms. "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--out-dir", type=Path, default=Path("gw_imr_nsims_logprob_sweep"))
    ...
    parser.add_argument(
        ...
        type=str,
        default="IMRPhenomD",
        help="pycbc frequency-domain approximant to use (e.g. IMRPhenomD, IMRPhenomXAS).",
    )
    parser.add_argument(
        "--f-high",
        type=float,
        default=2048.0,
        help=(
            "Upper frequency cutoff [Hz] for the common grid. Default 2048 resolves "
            "IMRPhenomD ringdown for all masses in [5, 50] M_sun; set to ~440 to mimic "
            "the TaylorF2 f_ISCO cutoff (ringdown-ablation study)."
        ),
    )
    parser.add_argument(
        "--n-pca",
        type=int,
        default=40,
        help="Number of PCA components retained for the compressed observation summary.",
    )
    parser.add_argument(
        "--fom-nsims",
        type=int,
...
def frequency_grid(cfg: GwWaveformConfig) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    freqs = np.arange(cfg.f_low, cfg.f_high, cfg.df)
    psd = aligo_psd(freqs)
...
def imrphenomd_waveform(
    m1: float,
    m2: float,
    freqs: np.ndarray,
    cfg: GwWaveformConfig,
) -> np.ndarray:
    """IMRPhenomD h_+ frequency-domain strain sliced onto the common ``freqs`` grid.

    Uses pycbc's ``get_fd_waveform`` so the full inspiral-merger-ringdown is
    retained. Non-spinning (spin1z = spin2z = 0). pycbc requires mass1 >= mass2,
    so masses are swapped internally; IMRPhenomD is symmetric under that swap
    for aligned spins (both zero here), so swapping is lossless.
    """
    ...
    hp, _ = get_fd_waveform(
        ...
        f_lower=cfg.f_low,
        f_final=cfg.f_high,
        distance=cfg.d_l_mpc,
    )
    h_full = np.asarray(hp.data, dtype=complex)
    n_freq = freqs.size
    i_start = int(round(cfg.f_low / cfg.df))
...
def clean_waveform_vector(
    ...
    h = imrphenomd_waveform(float(theta[0]), float(theta[1]), freqs, cfg)
    hw = h * whiten_factor
...
def main() -> None:
    ...
    print(f"Approximant: {cfg.approximant}")
