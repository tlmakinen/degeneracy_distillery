...
def reuse_fishnet_ensemble(data, from_dir: Path, outdir: Path):
    ...
    deterministic given the master seed. That is exactly why it is safe -- and
    it is checked rather than assumed: the scaled theta implied by the reused
    artifacts must match the fresh one, otherwise the reused Fishers belong to
    a different dataset and every downstream number would be silently wrong.
    """
    scaler = fit_theta_scaler(data["theta_train"], feature_range=(1.0, 2.0))
    ...
    fish_dir = from_dir / "fishnets-kuramoto"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help="Reuse the trained fishnet ensemble from --from-dir instead of "
             "retraining it. The simulator is still re-run (it is cheap and the "
             "theta scaler is derived from it), and the reused artifacts are "
             "checked against the fresh simulation before use. Intended for "
             "flattener/SR sweeps, where holding the Fisher stage fixed is what "
