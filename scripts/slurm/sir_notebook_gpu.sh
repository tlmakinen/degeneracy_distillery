...
# Batch runner for tutorial_notebooks/sir_example.ipynb converted to
# scripts/sir_notebook_run.py. Defaults to a shorter monitored smoke run;
# use MODE=full for the notebook-sized run, MODE=rebuttal for the NeurIPS
# rebuttal configuration (n_train=500, matching the notebook's own tuned
# default; SR augmentation stays at the notebook's frozen n_sr=10000).
#
# Launch as a seed array (one seed per task, master seed = array task ID):
#   sbatch --array=0-9 scripts/slurm/sir_notebook_gpu.sh
# MODE/OUT_BASE/etc. are still overridable via env, e.g.:
#   MODE=rebuttal sbatch --array=0-9 scripts/slurm/sir_notebook_gpu.sh
...
OUT_BASE="${OUT_BASE:-${SCRATCH:-$REPO_DIR/follow_up_results}/sir/$MODE}"
MIN_R0_CORR="${MIN_R0_CORR:-0.5}"
