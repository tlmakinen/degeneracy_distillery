...
# rebuttal configuration (n_train=500, n_aug=2000).
#
# Launch as a seed array (one seed per task, master seed = array task ID):
#   sbatch --array=0-9 scripts/slurm/rosenbrock_notebook_gpu.sh
# MODE/OUT_BASE/etc. are still overridable via env, e.g.:
#   MODE=rebuttal sbatch --array=0-9 scripts/slurm/rosenbrock_notebook_gpu.sh
...
OUT_BASE="${OUT_BASE:-${SCRATCH:-$REPO_DIR/follow_up_results}/rosenbrock/$MODE}"
MIN_ROSEN_CORR="${MIN_ROSEN_CORR:-0.5}"
MIN_THETA1_CORR="${MIN_THETA1_CORR:-0.5}"
...
python scripts/rosenbrock_notebook_run.py \
  ...
  --min-theta1-corr "$MIN_THETA1_CORR"
